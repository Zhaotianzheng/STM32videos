...
float ADCData; //raw ADC1 data
char buffer[10]; //buffer for float number
...
int main(void)
{
  ...
  while(1)
  {
    while(!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC)); // wait for data
    ADCData = (float)ADC_GetConversionValue(ADC1);
    ADCData = ADCData * 3.3 / (float)(1 << 12); // convert to volts
    sprintf(buffer, "%f", ADCData); // put float number into a string
    USART_SendText(buffer); // send number
    USART_SendText("\n");
  }
}
...
/* This function sets up ADC1 peripheral */
void ADC_Setup()
{
  ADC_InitTypeDef ADC_InitStruct;
  ADC_CommonInitTypeDef ADC_CommonInitStruct;

  // Setup global settings for ADCs
  ...
  ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1, ADC_SampleTime_144Cycles); // use this pin
